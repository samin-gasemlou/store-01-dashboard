...
export default function SelectInput({ label, value, options = [], onChange }) {
  ...
  return (
    <div className="flex flex-col gap-1 sm:gap-2 text-right w-full sm:w-61.25 relative">
      <label className="text-xs sm:text-sm md:text-[13px] font-medium text-right py-2">
{label}</label>
      <div
        onClick={() => setOpen(p => !p)}
        className="h-11 sm:h-13.75 px-3 sm:px-4 rounded-xl border border-[#0000004D] flex items-center justify-between text-xs sm:text-sm text-right w-full bg-[#ffffff] cursor-pointer"
      >
        <span>{selected}</span>
